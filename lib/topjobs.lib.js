...
requestRetry(request, { retryDelay: requestRetry.exponentialDelay });
...
class TopJobs {
  ...
  getAttachment(link) {
    if (/JobAdvertismentServlet/i.test(link)) {
      link = link.replace(
        "/JobAdvertismentServlet",
        "/advertismentpreview.jsp"
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      request
        ...
        .then(response => {
          ...
          debug(`method[getAttachment] <- ${link} is unknown`);
          return resolve({
            type: "unknown"
          });
        })
